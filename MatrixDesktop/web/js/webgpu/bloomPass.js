...
export default ({ config, device }) => {
	...
	const run = (encoder, shouldRender) => {
		...
		for (let i = 0; i < pyramidHeight; i++) {
			const levelWidth = Math.floor(scaledScreenSize[0] * 2 ** -i);
			const levelHeight = Math.floor(scaledScreenSize[1] * 2 ** -i);
			const dispatchSize = [Math.ceil(levelWidth / 32), levelHeight, 1];
